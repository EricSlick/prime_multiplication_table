...
1. Familiarize and Analyze: analyze the problem, research parts that are unfamiliar to get a general idea of what it will take to solve<br>
  * Identify and deal with parts of the requirements that I'm unfamiliar with<br>
    * Unfamiliar with logarithms surrounding finding prime numbers so spent a good deal of time researching this till I felt comfortable I understood enough to make a reasonable decision<br>
      * http://en.wikipedia.org/wiki/Sieve_of_Atkin<br>
        * this looks promising<br>
      * http://mathforum.org/isaac/problems/prime2.html<br>
      * http://www.gamedev.net/topic/403264-solved-program-to-test-if-a-number-is-prime/ <br>
        * Under Wiggin's comment, this one looks fast for larger values of N and is an implementation of the Sieve of Atkin? http://en.wikipedia.org/wiki/Sieve_of_Atkin
      * I researched more sites, but decided it wasn't useful to list them all
      * I spent some time doing thought experiments to see if I could reason out variants on the solutions I studied.
      * Decided that Wiggin's example would be the right solution to start with. Not necessarily the best, but have reasonable expectations it will be fine. Can change later as needed.
  * Story Map User Interaction with Stakeholder
    * This is small enough that I don't really need to do this. But for demonstration purposes, I'm showing soething here I like to do whenever possible since it creates greater certainty between the product owner (non-coder) and the programmer on what is being built and what it will (roughly) look like to the end user
      * User Requirements
       * from command line, input a command with an optional parameter
       * the return is a formatted table showing the prime numbers as a multiplications table. The default is 10 prime numbers
      * a gem (easy to set up, a bit easier to implement tests and allows reuse and running from the command line)
        * doing this one since it is easy to set up a gem and will likely save time than setting rspec up manually
...
* Step 2: Interface is the Command Line (ie, the Terminal or Console)
  * User Input
    * From Terminal/Console Type './bin/prime_mult_table <value>'  (value is optional -- default is 10)
      * User Output
        * a formatted multiplications table is printed to STDOUT (to the console/terminal)
          * though the look of the table is not directly specified, it's presumed that user comfort is expected. This should not result in a hard to read table. Formatting should be aligned and use ways to help the eye find the right prime and the right multiple
        * User Running Tests
          * type rspec
...
5. TDD (test first ... red/green/refactor/green)
  ...
  * Version 2: 
    * Consider solutions to formatting issues with output on N > a reasonable screen width
